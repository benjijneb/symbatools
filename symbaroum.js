			...
			filtered = items.filter(item => {
				...
				if (item.epingle || item.epinglen || item.epinglea || item.epinglem) {
					...
					tokens.forEach(token => {
						...
						var name = (undefined != item[query.lang] && undefined != item[query.lang].nom) ? item[query.lang].nom : item["en"].nom;
						var tradition = (undefined != item[query.lang] && undefined != item[query.lang].tradition) ? item[query.lang].tradition : item["en"].tradition;
...
//TODO Split Search Page and Build page
...
routeAppControllers.controller('Recherche', function ($scope, $http, $q, $routeParams, $rootScope, $translate, globalService, auth) {
	...
	$scope.importer = function ($fileContent) {
		...
		if ($fileContent) {
			...
			jsonImporter.epingles.forEach(tmp => {
				var i = $scope.talents.findIndex(getIndex, { "id": tmp, "version": jsonImporter.format, "lang": jsonImporter.lang });
	...
	$scope.calculsStatBlock = function($resetIntegrated) {
		...
		$scope.talents.forEach(talent => {
			...
			var type = talent.type;
			var nom = talent[$rootScope.lang].nom;
